...
def to_telegraph_link(text, chat_id):
  ...
  ex_command(
    "update bot_users set last_notes = '" + str(
      'http://telegra.ph/{}'.format(response['path'])) + "' where chat_id = '" + str(chat_id) + "';")
...
def last_weekly_grates(chat_id):
  timelist = data_command(
    "SELECT note, date FROM notes WHERE date > CURRENT_DATE - INTERVAL 7 DAY AND chat_id = '" + str(
      chat_id) + "' ORDER BY date DESC;")
  return gratelist_to_str(timelist)
